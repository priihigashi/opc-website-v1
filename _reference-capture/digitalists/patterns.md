...
> 6. **The fixed layer must NEVER overlay other sections.** A `position:fixed` pin layer must be active ONLY while the strip fully covers the viewport (`strip.top<=0 && strip.bottom>=vh`) — NOT the instant the strip merely peeks in. (Binary IntersectionObserver `threshold:0` snapped the fixed layer over the HERO and broke when the lab banner was hidden — real bug, fixed 2026-06-26.)
