...
import HouseModelV14 from "./HouseModelV14";
...
export default function HouseSceneV15() {
  return (
    <div className="fixed inset-0 z-0" data-testid="house-scene" aria-hidden>
      ...
      <Canvas shadows="soft" dpr={[1, 1.75]} camera={{ fov: 38, position: [2.1, 4.75, 14.8] }} gl={{ antialias: true }} onCreated={({ camera }) => { camera.lookAt(0, 1.15, 0); window.__cam = camera; }}>
        ...
        <Suspense fallback={null}><HouseModelV14 /></Suspense>
        ...
        <EffectComposer multisampling={4}>
          <N8AO intensity={0.78} aoRadius={0.72} distanceFalloff={1.8} quality="medium" />
        </EffectComposer>
      </Canvas>
    </div>
  );
