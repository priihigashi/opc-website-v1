...
// V32 preserves V31 and routes only through the versioned V30 model.
export default function HouseSceneV32({ onFrame = null }) {
  return (
    <div className="fixed inset-0 z-0" data-testid="house-scene" aria-hidden>
      ...
      <Canvas shadows="soft" dpr={[1, 1.7]} camera={{ fov: 38, position: [2.1, 4.75, 14.8] }} gl={{ antialias: true, powerPreference: "high-performance", toneMapping: THREE.ACESFilmicToneMapping, toneMappingExposure: 0.94 }} onCreated={({ camera, gl }) => { camera.lookAt(0, 1.15, 0); gl.shadowMap.type = THREE.PCFSoftShadowMap; gl.localClippingEnabled = true; window.__cam = camera; }}>
