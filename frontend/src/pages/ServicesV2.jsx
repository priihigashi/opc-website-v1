...
import { SERVICES_V2, SERVICES_V2_RESTING_PROGRESS } from "./servicesDataV2";
...
export default function ServicesV2() {
  ...
  const pick = (service) => {
    ...
    const start = scrollStore.p;
    const duration = reducedMotion ? 420 : service.duration * 1000;
    ...
    const tick = (now) => {
      ...
      scrollStore.p = start + (service.target - start) * eased;
      ...
      <section className="mx-auto flex min-h-[100svh] w-full max-w-[110rem] flex-col px-4 pb-5 pt-20 sm:px-6 md:px-8 md:pb-8 md:pt-24 xl:px-12">
        <div className="services-heading-v2 pointer-events-auto mx-auto w-full max-w-4xl text-center">
          ...
          <p className="mx-auto mt-3 max-w-2xl text-xs leading-relaxed text-[#C8C7C1] sm:text-sm">
            Select a service to see the house reveal the work, then explore the full service page.
          </p>
        </div>
        ...
        <div className="mt-3 min-h-9 text-center" aria-live="polite">
          {selected ? (
            <div data-testid="preview-progress-v2" className="pointer-events-auto mx-auto w-full max-w-sm">
              <p className="font-mono text-[10px] uppercase tracking-[0.24em] text-[#EEEDE9]">
                Revealing {selected.label}
              </p>
            ...
            <p className="font-mono text-[9px] uppercase tracking-[0.24em] text-[#8F8F94] sm:text-[10px]">
              Every service remains available at its own direct, indexable URL.
            </p>
          )}
        </div>
      </section>
