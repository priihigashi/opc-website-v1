...
export default function PortfolioV3() {
  ...
  return <div data-testid="portfolio-page" className="min-h-screen bg-[#09090B] pt-16 text-[#FAFAFA]">
    ...
    <nav aria-label="Portfolio categories" className="sticky top-16 z-30 border-y border-white/10 bg-[#09090B]/82 backdrop-blur-xl"><div className="no-scrollbar mx-auto flex max-w-7xl gap-2 overflow-x-auto px-6 py-4 md:px-10">{PORTFOLIO_FILTERS.map((item) => <button key={item} type="button" aria-pressed={filter === item} onClick={() => setFilter(item)} className={`flex-none rounded-full border px-4 py-2 font-mono text-[9px] uppercase tracking-[0.18em] ${filter === item ? "border-[#CBCC10] bg-[#CBCC10] text-[#09090B]" : "border-white/15 text-[#AFAFB4] hover:text-white"}`}>{item}</button>)}</div></nav>
