...
const getDisplayLabel = (label) => {
  ...
  const sequence = segments.pop() || label;
  return {
    scope: segments.join(" · ") || null,
    sequence: sequence.replaceAll("During Construction", "During"),
  };
};
...
function GalleryRow({ row, rowIndex, projectTitle, aboveFold = false }) {
  ...
  const displayLabel = getDisplayLabel(row.label);
      ...
      <div className="mx-auto flex w-full max-w-[880px] items-end justify-between gap-6 px-6 md:px-10">
        <div className="min-w-0 flex-1">
          ...
          <h2
            ...
            className={`${aboveFold ? "mt-1.5" : "mt-3"} font-head uppercase leading-none`}
          >
            {displayLabel.scope && (
              <span aria-hidden="true" className="mb-1.5 block font-mono text-[9px] tracking-[0.24em] text-white/55 sm:text-[10px]">
                {displayLabel.scope}
              </span>
            )}
            <span aria-hidden="true" className="block whitespace-nowrap text-[clamp(1.35rem,6.25vw,3rem)]">
