...
import DeferredHouseStageV7 from "@/components/DeferredHouseStageV7";
import HouseSceneV28 from "@/three/HouseSceneV28";
import NavV4 from "@/components/NavV4";
import StoryV18 from "@/components/StoryV18";
...
function Landing({ Stage = null }) {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      scrollStore.p = clamp01(-rect.top / Math.max(1, rect.height - window.innerHeight));
    };
    if (!reduced) {
  ...
  return (
    <div className="bg-[#09090B] font-body text-[#FAFAFA] antialiased">
      ...
      {Stage ? <Stage /> : <DeferredHouseStageV7 />}
      <main className="relative z-10">
        <StoryV18 storyRef={storyRef} />
        <div className="relative border-t border-white/10 bg-[#09090B]">
          <MarqueeV2 /><AboutV3 /><GalleryV4 /><TestimonialsV3 /><ContactV7 /><FooterV3 />
        </div>
      </main>
    </div>
  );
...
function ServicesStageGate() {
  ...
  useEffect(() => {
    if (active) { setShow(true); return undefined; }
    const id = setTimeout(() => setShow(false), 800);
    return () => clearTimeout(id);
  }, [active]);
  return show ? <DeferredHouseStageV7 scene="services" /> : null;
}

// V9 preserves AppV8 and adds Recent Project imagery inside each homepage service card.
