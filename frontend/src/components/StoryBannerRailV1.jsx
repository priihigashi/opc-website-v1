...
import { ChapterCardV1 } from "./ChapterV4";
...
export default function StoryBannerRailV1({ chapters }) {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (panel && chapter && nextIndex === renderedIndexRef.current) {
        ...
        if (isReadable !== readableRef.current) {
  ...
  return (
    <div data-testid="story-banner-rail" className="pointer-events-none fixed inset-y-0 left-1/2 z-20 grid w-full max-w-7xl -translate-x-1/2 grid-cols-12 items-start px-[max(1rem,env(safe-area-inset-left))] min-[768px]:px-6 min-[1100px]:px-10">
      <div
        ref={panelRef}
        className={`col-span-12 mx-auto w-[calc(100%_-_2rem)] max-w-[430px] will-change-transform min-[768px]:mx-0 min-[768px]:w-[min(36vw,360px)] min-[1100px]:w-[clamp(340px,28vw,430px)] ${lane}`}
        style={{ transform: "translate3d(0, calc(100svh + 8px), 0)" }}
      >
        <ChapterCardV1 {...chapter} panelRef={null} interactive={readable} />
      </div>
    </div>
  );
}
