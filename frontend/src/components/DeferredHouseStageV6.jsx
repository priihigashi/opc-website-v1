...
const HomeHouse = lazy(() => import("@/three/HouseSceneV29"));
...
/**
 * Candidate-only stage wrapper. Lifecycle and failure behavior are preserved from
 * V5. The only visual difference is the mobile home composition: the interactive
 * model comes from HouseSceneV29 and the static fallback mirrors its smaller,
 * upper-zone placement so banners do not cover the focal house at chapter focus.
 */
  ...
  return (
    <>
      <div
        ...
        data-static-reason={staticReason(staticOnly, interactiveFailed)}
      >
        {showStatic ? (
          <ResponsiveImageV1
            ...
            height="900"
            fetchPriority="high"
            className={`h-full w-full object-contain object-center [mask-image:radial-gradient(ellipse_62%_58%_at_center,black_58%,transparent_92%)] ${fallbackComposition}`}
          />
        ) : null}
      </div>
      {!showStatic ? (
