...
function RecentProjectV1({ id, image }) {
  return (
    <div data-testid={`${id}-recent-project`} className="mt-3 border-t border-white/10 pt-3">
      ...
      <div className="mt-2.5 aspect-video overflow-hidden rounded-lg border border-white/10 bg-black/20">
        {image.widths ? (
          <PortfolioPicture
            image={image}
...
export function ChapterCardV2({
  ...
  return (
    <div
      ...
      className="story-copy-panel pointer-events-auto w-full p-4 sm:p-5 min-[1100px]:p-6"
    >
...
/** V5 keeps reduced-motion chapters in normal document flow with the project image inside the card. */
export function ChapterV5({ side, timing, ...cardProps }) {
    ...
    <section id={cardProps.id} data-testid={cardProps.id} className="pointer-events-none relative flex min-h-[100svh] items-end px-4 pb-5 pt-24 min-[768px]:items-center min-[768px]:px-6 min-[1100px]:px-10">
      <div className="mx-auto grid w-full max-w-7xl grid-cols-12">
        <div className={`col-span-12 mx-auto w-full max-w-[430px] min-[768px]:mx-0 min-[768px]:w-[min(36vw,360px)] min-[1100px]:w-[clamp(340px,28vw,430px)] ${lane}`}>
          <ChapterCardV2 {...cardProps} interactive />
        </div>
      </div>
    </section>
  );
