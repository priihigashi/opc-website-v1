...
export default function StoryBannerRailV3({ chapters }) {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (panel && chapter && nextIndex === renderedIndexRef.current) {
        ...
        const isReadable = y >= 86 && y + panelHeight <= window.innerHeight - (compactDock ? 48 : 8);
  ...
  return (
    <div data-testid="story-banner-rail" className="pointer-events-none fixed bottom-12 top-0 left-1/2 z-20 grid w-full max-w-7xl -translate-x-1/2 grid-cols-12 items-start overflow-hidden px-[max(1rem,env(safe-area-inset-left))] min-[768px]:px-6 min-[1100px]:px-10 xl:bottom-0 xl:overflow-visible">
