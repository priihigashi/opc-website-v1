...
test('Candidate 5 preserves the approved project set and exact displayed counts', () => {
 ...
 assert.equal(projects.reduce((n,p)=>n+p.imageCount,0),67);
...
test('documented unfinished photographs cannot regain Finished labels', () => {
 ...
 assert.deepEqual(projects.filter(p=>p.progressOnly).map(p=>p.id).sort(),['dockside-full-home-remodel','pompano-kitchen-remodel','shell-concrete-construction']);
 assert.match(read('../pages/PortfolioV9.jsx'),/project.progressOnly \? "Construction progress/);
});
test('current gallery has one identity, contained 16:9 photographs and the Candidate 4 navigation owner',()=>{
 const gallery=read('../pages/ProjectGalleryV6.jsx');
 ...
 assert.match(gallery,/<ProjectGalleryV5><GalleryContent \/><\/ProjectGalleryV5>/);
 ...
 assert.doesNotMatch(read('../AppV18.js'),/preview\/centered-house/);
});
