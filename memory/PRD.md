...
- DONE (v3 polish): facade window/cutout alignment fix (ribbon window vs wood screen overlap) in both the R3F parts and the GLB factory (re-exported residence.glb v4); contact email = contact@oakpark-construction.com; Selected Work section is now a full OPC-lime contrast block with dark text
