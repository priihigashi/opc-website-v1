...
- DONE (v5): pool chapter (lap pool fills beside pergola, landing + GLB); powder-room toilet replaces tub; upstairs full bathroom (vanity, walk-in shower, toilet, wet-zone tile); partition walls capped at ceiling lines (no more leaking through floors) and fade out during the cutaway (tileBath/partWhite driven by cut); roof A membrane leveled flush with coping; wood screen measured exactly (canopy top 3.36m → 6.02m below coping, door width 1.2m); pavilion roof trimmed to die into the block wall (no more front-wall overlap); GLB re-exported (v11)
