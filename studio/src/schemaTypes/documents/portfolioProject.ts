...
export const portfolioProject = defineType({
  ...
  preview: {
    select: { title: "title", subtitle: "primaryCategory", archived: "archived", media: "sequences.0.images.0.asset" },
    prepare: ({ title, subtitle, archived, media }) => ({ title: archived ? `ARCHIVED — ${title}` : title, subtitle, media }),
  },
});
